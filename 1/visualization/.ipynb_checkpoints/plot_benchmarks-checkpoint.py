...
def load_csv(csv_path: Path) -> pd.DataFrame:
    ...
    if df.empty:
        raise ValueError(f"{csv_path.name}: после фильтрации не осталось валидных строк")

    return df


def build_summary(df: pd.DataFrame):
    summary = df.groupby('threads')['time'].mean() 
    summary["threads"]
    summary["speedup"]

    return summary

def plot_runtime(df: pd.DataFrame, summary: pd.DataFrame, title: str, out_png: Path) -> None:
    plt.figure(figsize=(10, 6))

    plt.scatter(df["threads"], df["time"], alpha=0.5, label="Отдельные прогоны")
    plt.plot(summary["threads"], summary["mean_time"], marker="o", label="Среднее время")
...
def plot_speedup(summary: pd.DataFrame, title: str, out_png: Path) -> None:
    ...
    plt.plot(summary["threads"], summary["speedup"], marker="o", label="Измеренное ускорение")
    plt.plot(summary["threads"], summary["threads"], linestyle="--", label="Идеальное ускорение")
...
def process_csv(csv_path: Path, output_dir: Path) -> None:
    ...
    title = sanitize_name(csv_path.stem)
